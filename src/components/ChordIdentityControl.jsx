...
function ChordPartControl({ id, className, label, value, placeholder, previousLabel, nextLabel, onPrevious, onNext, onCommit }) {
  ...
  return (
    <span className={'chord-identity-part ' + className}>
      <input id={id} className="chord-identity-input" aria-label={label} aria-invalid={Boolean(error)}
        ...
        size="1" style={{ '--chord-input-chars': Math.max(draft.length, 1) }} autoCapitalize="none" autoComplete="off" spellCheck="false"
...
function ChordIdentityControl({ root, suffix, index, availableSuffixes, onPreviousRoot, onNextRoot, onPreviousSuffix, onNextSuffix, onCommitChord, onCommitSuffix }) {
  ...
  const commitRoot = (draft) => {
    ...
    onCommitChord(parsed.key, parsed.suffix);
    return true;
  };
  ...
  return (
    <div className="chord-identity-control" aria-label={'Acorde ' + chordName + ': ' + qualityLabel} title={'Acorde ' + chordName + ': ' + qualityLabel}>
      <ChordPartControl id={'chord-root-' + index} className="chord-identity-root" label={'Nota do acorde ' + step}
        value={root} previousLabel={'Nota anterior do acorde ' + step} nextLabel={'Próxima nota do acorde ' + step}
