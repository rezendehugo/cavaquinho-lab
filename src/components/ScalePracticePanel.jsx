  ...
  return <div className="scale-practice-panel scale-mode path-view">
    <div className="fretboard-control-panel">
      <div className="scale-explorer">
          ...
          {!editing ? <div className="path-actions"><button type="button" data-ui-text-reason="domain-choice" aria-label={hasValidPath ? 'Alterar início e fim' : 'Escolher início e fim'} title={hasValidPath ? 'Alterar início e fim' : 'Escolher início e fim'} onClick={() => resetForSelection('start')}>{hasValidPath ? 'Alterar início e fim' : 'Escolher início e fim'}</button><button type="button" data-ui-text-reason="workflow" onClick={saveCurrentPath} disabled={!hasValidPath}>Salvar caminho</button><button type="button" data-ui-text-reason="workflow" onClick={startEditing} disabled={!hasValidPath}>Editar caminho</button>{selectedPath.custom ? <><input aria-label="Nome do caminho selecionado" value={draftName} onChange={event => setDraftName(event.target.value)} /><button type="button" data-ui-text-reason="workflow" onClick={renameSelected}>Salvar nome</button><button type="button" aria-label="Excluir caminho" title="Excluir caminho" onClick={deleteSelected}><Trash2 size={16} aria-hidden="true" /></button></> : null}</div>
            : <div className="path-editor"><p>{Number.isInteger(editIndex) ? 'Escolha outra posição para ' + draftPositions[editIndex].note + draftPositions[editIndex].octave + '.' : 'Escolha uma nota intermediária para editar.'}</p><div className="edit-step-list">{draftPositions.slice(1, -1).map((position, index) => <button type="button" key={positionKey(position)} aria-pressed={editIndex === index + 1} onClick={() => setEditIndex(index + 1)}>{position.note}{position.octave} · casa {position.fret}</button>)}</div><label><span>Nome</span><input aria-label="Nome do caminho" value={draftName} onChange={event => setDraftName(event.target.value)} /></label><div><button type="button" onClick={() => { const previous = draftHistory.at(-1); if (previous) { setDraftPositions(previous); setDraftHistory(history => history.slice(0, -1)); } }} disabled={!draftHistory.length}>Desfazer</button><button type="button" onClick={() => { setDraftPositions([]); setEditIndex(null); }}>Limpar</button><button type="button" onClick={() => { setEditing(false); setDraftPositions([]); setEditIndex(null); }}>Cancelar</button><button type="button" data-ui-text-reason="workflow" onClick={saveDraft} disabled={!validateCustomScalePath({ root: scaleRoot, scaleId, start: draftPositions[0], end: draftPositions.at(-1), positions: draftPositions }).ok}>Salvar caminho</button></div></div>}
          <div className="scale-practice"><div><BpmInput value={metronome.bpm} onChange={metronome.setBpm} ariaLabel="BPM da prática de escala" variant="practice" /><small>{directionLabels[direction]} · contagem de {countIn} tempos</small></div><button ref={practiceButtonRef} type="button" data-ui-text-reason="workflow" className="scale-practice-button" onClick={startPractice} disabled={editing || !hasValidPath}><Play aria-hidden="true" size={16} />Praticar escala</button></div>
          <p className="anchor-status" role="status">{anchorError || 'Início: ' + (effectiveStart?.note || '—') + ' · fim: ' + (effectiveEnd?.note || '—')}</p>{anchorError.includes('Não há caminho') ? <button type="button" data-ui-text-reason="workflow" onClick={useFullFretboard}>Usar braço inteiro</button> : null}
