...
describe('Cavaquinho Lab', () => {
  ...
  test('setas alteram nota, sufixo e forma do card selecionado', () => {
    ...
    expect(nextSuffix.closest('.chord-identity-control')).not.toBe(null);
    expect(nextSuffix.closest('.chord-identity-part')).toBe(null);
  ...
  test('adiciona, remove e reordena acordes preservando cards', async () => {
    renderAt();
    fireEvent.click(screen.getByRole('button', { name: 'Adicionar acorde' }));
    fireEvent.click(screen.getByRole('button', { name: 'Adicionar acorde' }));
    ...
    const reorderHandles = screen.getAllByRole('button', { name: 'Reordenar acorde' });
    expect(within(screen.getByLabelText('Acordes da sequência')).getAllByRole('article')[0]).not.toHaveAttribute('draggable');
    fireEvent.keyDown(reorderHandles[1], { key: 'ArrowLeft' });

    await waitFor(() => expect(screen.getByLabelText('Sequência atual')).toHaveTextContent('Db → C'));
